...
void RsaCrypto::rsaGenerateKey(int bits, string pub, string pri)
{
	RSA* rsa = RSA_new();
	//准备BIGNUM对象
	BIGNUM* bn = BN_new();
...
string RsaCrypto::rsaEncryptPublicKey(string data)
{
	//计算数据长度
	int len = data.size();
	//准备加密后空间
...
string RsaCrypto::rsaDecryptPublicKey(string data)
{
	//计算数据长度
	int len = data.size();
	char* buf = new char[1024];
...
string RsaCrypto::signPrivateKey(string data)
{
	//准备签名后的数据空间
	char* buf = new char[1024];
	unsigned int len;
...
bool RsaCrypto::verifyPublicKey(string data, string sigbuf)
{
...
bool RsaCrypto::initpublicKey(string fileName)
{
	//加载公钥信息
...
bool RsaCrypto::initprivateKey(string fileName)
{
	//加载私钥信息
	BIO* bio = BIO_new_file(fileName.data(), "r");
